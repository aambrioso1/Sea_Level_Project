# \Sea_Level_Project\sea_level.py
...
Things to do
************
Figure out how far back in time the data goes (MLLW).
Extract all the data for one station (St. Petersburg, Tampa Bay).
Compute the range of data.
Plot the data over time.
Compute a linear regression to calculate the change in height per year. 
Extend technique to at least 9 more stations.

Tampa Bay Operational Forecasts Systems (TBOFS)
...
MLLW: https://en.wikipedia.org/wiki/Chart_datum#Mean_Lower_Low_Water
MHHW: https://en.wikipedia.org/wiki/Chart_datum#Mean_Higher_High_Water

Typical MLLW
{'metadata': {'id': '8726520', 'name': 'St. Petersburg, Tampa Bay', 'lat': '27.7606', 'lon': '-82.6269'},
 'data': [{'t': '2021-06-12 14:00', 'v': '3.094', 's': '0.033', 'f': '0,0,0,0', 'q': 'p'}, 
 {'t': '2021-06-12 14:06', 'v': '3.094', 's': '0.043', 'f': '0,0,0,0', 'q': 'p'}]}
...
https://tidesandcurrents.noaa.gov/publications/Tech_rpt_53.pdf (a report on Sea Level Varation in the US)
A Excel spreadsheet of data from this report is saved in the sea_level_project folder.
"""

# imports need along with additional ones for plotting.
...
part3 = f'&product=water_level&datum=MLLW&units=english&time_zone={zone2}' # for collected mean sea level
# part3 = f'&product=water_temperature&units=english&time_zone={zone2}' # for collecting temperature data
...
REPEAT = len(TIME_STR) # compute the number of '*' needed. 
def pr():
...
"""
Earlier code for plotting temperature data.
time_list = []
temp_list = []
for item in d:
